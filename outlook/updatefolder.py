import pyOutlook

def outlook_update_folder(access_token, folder_name, new_name):
    try:
        account = pyOutlook.OutlookAccount(access_token)

        folders = account.get_folders()

        index = 0

        count = 0

        for i in range(0, len(folders)):

            if str(folders[i]) == folder_name:

                index = i

                break

            else:
                count = count + 1

        if count == len(folders):
            return {'status': 'Fail', 'error': 'The folder with the name '+ folder_name +' not found'}

        else:
            folders[index].rename(new_name)

            return {'status': 'Pass', 'error': None}

    except pyOutlook.internal.errors.AuthError as error:
        return {'status': 'Fail', 'error': 'Authentication failed'}

    except pyOutlook.internal.errors.APIError as error:
        return {'status': 'Fail', 'error': 'API error occurred'}

    except Exception as error:
        return {'status': 'Fail', 'error': str(error)}


access_token = ''
folder_name = 'Fame'
new_name = 'Name'

result = outlook_update_folder(access_token, folder_name, new_name)
print(result)


